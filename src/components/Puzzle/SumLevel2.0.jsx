...
const y = randomIntFromInterval({...interval, min: x})

const Puzzle = () => {
    const [nums, setNums] = useState({x, y})
    ...
    const generateEq = () => {
        return `${nums.descazut} ${randomOperator} ${answer ? answer : "?"} = ${nums.y}`
    }

    function calculate() {
        if (randomOperator === "+") {
            return nums.y - nums.descazut;
